...
while ( my $f = $dir->next ) {
    ...
    DEBUG 'Found file ' . $f;
my $file = file($run_dir, 'test_file.txt');
#say "File is $file";

    copy($f, $sub_dir);

   my @cmd = ( 'tar','cvzf', 'backup_files.tgz',  $sub_dir );
...
find_and_compress.pl - Populate the user database tables.
